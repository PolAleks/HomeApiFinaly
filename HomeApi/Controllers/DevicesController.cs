...
namespace HomeApi.Controllers
{
    ...
    public class DevicesController : ControllerBase
    {
        ...
        /// </summary>
        /// <param name="id">Guid удаляемого устройства</param>
        [HttpDelete("{id}")]
        public async Task<IActionResult> Delete([FromRoute] Guid id)
        {
            try
            {
                Device deleteDevice = await _devices.GetDeviceById(id);

                if (deleteDevice == null)
                {
                    return StatusCode(400, $"Устройство с идентификатором {id} не найдено, операция удаления невозможна.");
                }
                await _devices.DeleteDeviceAsync(deleteDevice);

                return StatusCode(200, $"Устройство {deleteDevice.Name} успешно удалено!");
            }
            catch
            {
                return StatusCode(400, $"Что-то пошло не так в процессе удаления устройства.");
            };
        }
